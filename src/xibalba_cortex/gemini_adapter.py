"""Gemini CLI wrapper shim.

Gemini CLI does not have a verified native hook surface in this codebase, so this adapter is
intentionally lifecycle-only, mirroring AgyWrapperShim: it binds identity, opens/closes
sessions, and emits best-effort telemetry around wrapper entry/exit. It does not pretend to
provide Claude-equivalent tool hooks.
"""
        ...
        **kwargs: Any,
        ...
        self.controller.ingest_event(
            RuntimeEvent(
                ...
                traceparent=traceparent,
                agent_id=agent_id,
                tool_name="gemini.wrapper.start",
    ...
    def record_observation(self, *, session_id: str | None = None, note: str | None = None, **kwargs: Any) -> dict[str, Any]:
        ...
        self.controller.ingest_event(
            RuntimeEvent(
                runtime=self.runtime,
                session_id=session_id,
                tool_name="gemini.wrapper.observation",
