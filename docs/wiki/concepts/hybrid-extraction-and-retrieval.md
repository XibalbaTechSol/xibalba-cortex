...
A live, unmocked round trip against a throwaway store completed an extraction task and produced three entities. The server validated the input snapshot hash, output schema, and evidence-quote containment before inserting reviewable `extraction_proposals`. Extraction remains proposal-only: acceptance is explicit, source memories are not mutated, and stale source hashes become `stale` rather than being promoted.

Two `extract_entities` results may legitimately differ when they came from different providers. `structural` is deterministic regex matching for URLs, paths, UUIDs, commit hashes, and code-fence languages; `hermes`, `in_session`, and custom providers perform semantic extraction. Every task now records `provider_id`, so an empty structural result is not presented as disagreement with a non-empty semantic result. Model confidence is only the provider assessed support for a proposal; it is not truth probability. Exact evidence-quote containment remains the common fail-closed gate. Live history-sweep failures reproduced the other apparent inconsistency: models sometimes normalize or invent a quote, and those results are rejected rather than silently corrected. The earlier diagnostic that returned unrelated recalled-context quotes remains a fail-closed isolation finding recorded in the dated audit trail.
