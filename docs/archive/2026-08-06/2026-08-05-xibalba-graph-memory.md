...
## Audit reconciliation — 2026-08-06

The repository-local audit ledger is [`docs/audits/2026-08-06-status.md`](../audits/2026-08-06-status.md). The normative v1 specification defines intended behavior; this implementation plan records sequencing; the audit ledger records observed state. Older plans under `docs/plans/`, architecture notes, and session logs remain historical design evidence unless the ledger explicitly promotes a task to `DONE`.

Observed status: the test suite passes when the declared `drive` extra is installed; a plain default install fails test collection because Drive tests import optional Google dependencies. The active worktree is three commits ahead and contains uncommitted runtime/controller/viewer changes. Those changes are not a default-branch capability claim until separately reviewed and committed.
