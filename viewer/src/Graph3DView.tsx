...
function nodePosition(node: DemoNode, index: number, total: number): THREE.Vector3 {
  if (node.type === 'merkle') return new THREE.Vector3(0, 95, 0)
  if (node.type === 'session') return new THREE.Vector3(0, 52, 0)
  const hash = stableHash(node.id)
  const ring = node.type === 'exchange' ? 46 : node.type === 'entity' ? 72 : 96
  ...
  useEffect(() => {
    ...
    for (const edge of graph.edges) {
      ...
      const material = new THREE.LineBasicMaterial({
        ...
        transparent: true,
        opacity: selected ? 0.96 : edge.type === 'similarity' ? 0.46 : 0.68,
      })
    ...
    const animate = () => {
      ...
      for (const [nodeId, mesh] of nodeObjects) {
        ...
        if (base) mesh.position.y = base.y + Math.sin(frame + stableHash(nodeId) * 0.001) * 1.1
