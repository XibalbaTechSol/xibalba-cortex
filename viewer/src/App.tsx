...
const tabs: Array<{ id: Tab; label: string }> = [
  ...
  { id: 'integrity', label: 'Integrity Audit' },
...
function CortexOverview({ stats, status, operations, sessions, onOpen }: { stats: Stats | null; status: StoreStatus | null; operations: OperationsSnapshot | null; sessions: Session[]; onOpen: (tab: Tab) => void }) {
  ...
  return <section className="cortex-overview-panel">
    ...
    <div className="overview-links"><button onClick={()=>onOpen('graph')}>Graph</button><button onClick={()=>onOpen('timeline')}>Replay timeline</button><button onClick={()=>onOpen('provenance')}>Provenance</button><button onClick={()=>onOpen('integrity')}>Integrity Audit</button></div>
...
function OperationsTab({ operations, onRefresh }: { operations: OperationsSnapshot | null; onRefresh: () => void }) {
  ...
  return <section className="tab-panel">
    ...
    <div className="card-grid">
      <section className="panel-card"><h3>Production readiness</h3><div className="badges"><Badge>{operations.production.state}</Badge><Badge>{operations.production.active_tokens + " active credentials"}</Badge></div><dl className="detail-list"><dt>Token lifecycle</dt><dd>{operations.production.token_lifecycle}</dd><dt>Tenant onboarding</dt><dd>{operations.production.tenant_onboarding}</dd><dt>Isolation</dt><dd>{operations.production.isolation_model}</dd></dl><h4>Open production gates</h4><ul className="small muted">{operations.production.open_gates.map((gate) => <li key={gate}>{gate}</li>)}</ul><p className="small muted">Local readiness evidence only. Tenant creation remains an operator-controlled action and is not exposed to browser sessions.</p></section>
